 ...
 *
 * @param activation_irq activation source.
 * @param p_src activation source.
 * @param p_dest activation source.
 * @param num_transfers activation source.
 *
 * @retval 0 On success.
 * @retval -EINVAL if activation source is invalid.
 */
int dtc_renesas_rx_reset_transfer(const struct device *dev, uint8_t activation_irq,
				  void const *volatile p_src, void *volatile p_dest,
 ...
 * @param dev DTC device instance.
 * @param activation_irq activation source.
 * @param transfer_properties_t Driver specific information.
 *
 * @retval 0 On success.
 * @retval -EINVAL if activation source is invalid.
 */
int dtc_renesas_rx_infoGet(const struct device *dev, uint8_t activation_irq,
