...
/**
 * @brief Respond to a received header (slave mode only).
 *
 * This is an interrupt-driven API for a slave node to respond to a LIN header sent by the
 * master node. For a slave node, it should be called when a LIN_EVT_RX_HEADER event occurs to
 * publish the response for the received header.
 *
 ...
 *
 * This is an interrupt-driven API that a slave node uses to initiate a non-blocking
