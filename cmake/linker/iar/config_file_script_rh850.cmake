...
function(process_region)
  ...
  foreach(section ${sections})
    ...
    if(NOT nosymbols)
      if(${name} STREQUAL .ramfunc)
        create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name_clean}_load_start
    ...
    if("${symbol_val}" STREQUAL "${name_clean}")
      create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name_clean}_size
      ...
      set_property(GLOBAL APPEND PROPERTY ILINK_REGION_SYMBOL_ICF
        "define image symbol __${name_clean}_size = (___${symbol_val} - ADDR(${name_clean}))")
    endif()
    set(ZI)
    ...
    if(${name_clean} STREQUAL last_ram_section)
      ...
      create_symbol(OBJECT ${REGION_OBJECT} SYMBOL __flash_used
        EXPR "(@LOADADDR(last_section)@ + @SIZE(last_section)@ - @___rom_region_start@)"
        )
    endif()
    ...
    if(${name_clean} STREQUAL rom_start)
      ...
      create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___Vectors
        EXPR "@ADDR(${symbol_start})@"
        )
      create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___vector_table
        EXPR "@ADDR(${symbol_start})@"
        )
    endif()
  ...
  foreach(group ${groups})
    ...
    if(DEFINED vma AND DEFINED lma)
      # Something to init
      create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name}_load_start
    ...
    create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name}_start
      EXPR "@ADDR(${first_section_name})@"
      )
    create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name}_end
      EXPR "@END(${last_section_name})@"
      )
    create_symbol(OBJECT ${REGION_OBJECT} SYMBOL ___${name}_size
      EXPR "(@(___${name}_end)@ - @(___${name}_start)@)"
      )
...
function(section_to_string)
  ...
  if(NOT nosymbols)
    if("${name_clean}" STREQUAL "tdata")
      ...
      set_property(GLOBAL APPEND PROPERTY ILINK_SYMBOL_ICF "___${name_clean}_start = __${name_clean}_start")
      set_property(GLOBAL APPEND PROPERTY ILINK_SYMBOL_ICF "__${name_clean}_end = END(${name_clean})")
      set_property(GLOBAL APPEND PROPERTY ILINK_SYMBOL_ICF "___${name_clean}_end = __${name_clean}_end")
    endif()
  endif()
  ...
  if("${name_clean}" STREQUAL "TP_BLOCK")
    set(TEMP "${TEMP}define block ${name_clean} with static base TP")
  else()
  ...
  foreach(idx idx_next IN ZIP_LISTS indicies next_indicies)
    ...
    if(DEFINED symbol_start)
      ...
      set_property(GLOBAL APPEND PROPERTY ILINK_SYMBOL_ICF "_${symbol_start} = ${symbol_start}")
    endif()
    ...
    if(any)
      ...
      foreach(flag ${flags})
        ...
        if("${flag}" STREQUAL +ZI)
          ...
          set_property(GLOBAL APPEND PROPERTY ILINK_CURRENT_SECTIONS "section ${ANY_FLAG}")
        endif()
      endforeach()
    ...
    if(DEFINED symbol_end)
      ...
      set_property(GLOBAL APPEND PROPERTY ILINK_SYMBOL_ICF "_${symbol_end} = ${symbol_end}")
    endif()
    if(${length} GREATER 0)
