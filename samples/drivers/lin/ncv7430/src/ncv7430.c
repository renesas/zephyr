...
static volatile uint32_t ncv7430_event_flags = 0;
...
int ncv7430_get_led_color(const struct device *dev, uint8_t addr, struct rgb_led_color *color)
{
	...
	ret = ncv7430_send_command(dev, NCV7430_MASTER_COMMAND, tx_data, NCV7430_8_BYTES);
	if (ret) {
		return ret;
	}
	 ...
	 * 	Byte 7: 0xFF (FILL)
	 */
	ret = ncv7430_request_response(dev, NCV7430_SLAVE_RESPONSE, rx_data, NCV7430_8_BYTES);
	if (ret) {
		return ret;
	}
