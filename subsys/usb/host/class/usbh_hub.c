...
static struct {
	...
	struct usbh_hub_data *processing_hub;
} hub_mgr;

static int hub_interrupt_in_cb(struct usb_device *const dev,
			       struct uhc_transfer *const xfer);

static int hub_resubmit_interrupt_in(struct usbh_hub_data *hub_data,
				     struct uhc_transfer *xfer)
{
	struct net_buf *buf;
	int ret;

	if (!hub_data->connected || (hub_data->int_ep == NULL)) {
...
resubmit:
	if (hub_data->connected && hub_data->state == HUB_STATE_OPERATIONAL) {
		ret = hub_resubmit_interrupt_in(hub_data, xfer);
		if (ret != 0) {
...
static void hub_print_info(struct usbh_hub_data *const hub_data)
{
...
static void hub_recursive_disconnect(struct usbh_hub_data *const hub_data)
{
	...
	for (uint8_t i = 0; i < hub_data->port_count; i++) {
		if (hub_data->port_list) {
			...
			if (hub_data->port_list[i].udev) {
				port_udev = hub_data->port_list[i].udev;
				child_hub = find_hub_by_udev(port_udev);
				if (child_hub != NULL) {
					LOG_DBG("Found child Hub on port %d, recursing", i + 1);
					hub_recursive_disconnect(child_hub);
				} else {
...
process_disconnection:
	if (port_instance->udev != NULL) {
		...
		child_hub = find_hub_by_udev(udev);
		if (child_hub != NULL) {
			LOG_DBG("Child Hub disconnected, triggering recursive removal");
			hub_recursive_disconnect(child_hub);
		} else {
...
exit_processing:
	...
	if (restart_interrupt && !hub_data->int_active && hub_data->connected) {
		ret = hub_start_interrupt(hub_data);
		if (ret) {
...
static void hub_process(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct usbh_hub_data *hub_data =
		CONTAINER_OF(dwork, struct usbh_hub_data, hub_work);
	...
	for (uint8_t i = 0; i < hub_data->port_count; i++) {
		...
		hub_data->port_list[i].speed = USB_SPEED_SPEED_FS;
		k_work_init_delayable(&hub_data->port_list[i].port_work, hub_port_process);
	}

	while (hub_data->port_index < hub_data->port_count) {
		hub_data->port_index++;

		LOG_DBG("Setting port %d power", hub_data->port_index);
		ret = usbh_req_set_hcfs_ppwr(hub_data->udev,
					     hub_data->port_index);
		if (ret != 0) {
			LOG_ERR("Failed to set port %d power: %d",
				hub_data->port_index, ret);
		} else {
			hub_data->port_list[hub_data->port_index - 1].state =
			  ...
			  const uint8_t iface)
{
	...
	hub_data->udev = udev;
	hub_data->uhs_ctx = hub_mgr.uhs_ctx;
	...
	k_work_init_delayable(&hub_data->hub_work, hub_process);
	...
	k_work_submit(&hub_data->hub_work.work);
