...
static void mtu_output_enable(const struct device *dev, int channel, uint8_t state)
{
...
static void pwm_rx_set_counter_clear(const struct device *dev, int counter_clear_channel)
{
	const struct pwm_renesas_rx_config *config = dev->config;
 ...
 */
static int pwm_rx_set_period(const struct device *dev, int channel, uint16_t period_cycles)
{
	const struct pwm_renesas_rx_config *config = dev->config;
	int counter_clear_channel;
	...
	pwm_rx_set_counter_clear(dev, counter_clear_channel);
...
static void mtu_start_counter(const struct device *dev)
{
	const struct pwm_renesas_rx_config *config = dev->config;
...
static void mtu_stop_counter(const struct device *dev)
{
	const struct pwm_renesas_rx_config *config = dev->config;
					    ...
					    void *user_data)
{
	...
	*config->reg.tmdr = TMDR_MD_PWM_NORMAL_MODE;
	...
	/* Set clear source */
	pwm_rx_set_counter_clear(dev, channel);
...
static const struct pwm_driver_api pwm_renesas_rx_driver_api = {
...
#define IRQ_PWM_INIT(index)	\
	...
	COND_CODE_1(DT_IRQ_HAS_NAME(DT_INST_PARENT(index), tgic),	\
		(.tgi_irq[RX_MTIOCxC] = DT_IRQ_BY_NAME(DT_INST_PARENT(index), tgic, irq),), ()) \
...
#define IRQ_PWM_CONFIG_INIT(index)	\
	do {	\
		IRQ_CONNECT(DT_IRQ_BY_NAME(DT_INST_PARENT(index), tgia, irq),	\
			    DT_IRQ_BY_NAME(DT_INST_PARENT(index), tgia, priority), \
				mtu_rx_tgi_isr, DEVICE_DT_INST_GET(index), 0);	\
		IRQ_CONNECT(DT_IRQ_BY_NAME(DT_INST_PARENT(index), tgib, irq),	\
			    DT_IRQ_BY_NAME(DT_INST_PARENT(index), tgib, priority),\
				mtu_rx_tgi_isr, DEVICE_DT_INST_GET(index), 0);	\
