...
static int uart_ra_sci_b_poll_in(const struct device *dev, unsigned char *c)
{
	...
	R_SCI_B0_Type *reg = data->sci.p_reg;

	/* Check if async reception was enabled */
	if (IS_ENABLED(CONFIG_UART_ASYNC_API) && reg->CCR0_b.RIE) {
		return -EBUSY;
	}

	if (IS_ENABLED(CONFIG_UART_RA_SCI_B_UART_FIFO_ENABLE) ? reg->FRSR_b.DR == 0U
							      : reg->CSR_b.RDRF == 0U) {
...
static int uart_ra_sci_b_apply_config(const struct uart_config *uart_config,
				      struct st_uart_cfg *fsp_config,
				      struct st_sci_b_uart_extended_cfg *fsp_config_extend,
				      struct st_sci_b_baud_setting_t *fsp_baud_setting)
{
	...
	fsp_err = R_SCI_B_UART_BaudCalculate(uart_config->baudrate, false, 5000, fsp_baud_setting);
...
static int uart_ra_sci_b_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	err = uart_ra_sci_b_apply_config(&data->uart_config, &data->fsp_config,
...
static int uart_ra_sci_b_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	...
	if (IS_ENABLED(CONFIG_UART_RA_SCI_B_UART_FIFO_ENABLE) && data->sci.fifo_depth > 0) {
		while ((size - num_tx > 0) && reg->FTSR != 0x10U) {
			/* FTSR flag will be cleared with byte write to TDR register */

			/* Send a character (8bit , parity none) */
			reg->TDR_BY = tx_data[num_tx++];
		}
	} else {
		if (size > 0 && reg->CSR_b.TDRE) {
...
static int uart_ra_sci_b_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	...
	if (IS_ENABLED(CONFIG_UART_RA_SCI_B_UART_FIFO_ENABLE) && data->sci.fifo_depth > 0) {
		while ((size - num_rx > 0) && reg->FRSR_b.R > 0U) {
			/* FRSR.DR flag will be cleared with byte write to RDR register */

			/* Receive a character (8bit , parity none) */
			rx_data[num_rx++] = reg->RDR;
		}
		if (reg->FRSR_b.R == 0U) {
			reg->CFCLR_b.RDRFC = 1U;
			reg->FFCLR_b.DRC = 1U;
		}
	} else {
		if (size > 0 && reg->CSR_b.RDRF) {
			/* Receive a character (8bit , parity none) */
			rx_data[num_rx++] = reg->RDR;
		}
	}
...
static void uart_ra_sci_b_irq_tx_enable(const struct device *dev)
{
	...
	reg->CCR0 |= (BIT(R_SCI_B0_CCR0_TIE_Pos) | BIT(R_SCI_B0_CCR0_TEIE_Pos));
}
...
static int uart_ra_sci_b_irq_tx_complete(const struct device *dev)
{
	...
	return (reg->CCR0_b.TEIE == 1U) && (data->csr & BIT(R_SCI_B0_CSR_TEND_Pos));
}
...
static int uart_ra_sci_b_irq_rx_ready(const struct device *dev)
{
	...
	return (reg->CCR0_b.RIE == 1U) &&
	       ((data->csr & BIT(R_SCI_B0_CSR_RDRF_Pos)) ||
		(IS_ENABLED(CONFIG_UART_RA_SCI_B_UART_FIFO_ENABLE) && reg->FRSR_b.DR == 1U));
}
...
static inline void disable_tx(const struct device *dev)
{
	...
	data->sci.p_reg->CCR0 &= (uint32_t) ~(R_SCI_B0_CCR0_TIE_Msk | R_SCI_B0_CCR0_TEIE_Msk);
	 ...
	 */
	while (data->sci.p_reg->CSR_b.TEND != 1U) {
	}

	data->sci.p_reg->CCR0 &= (uint32_t) ~(R_SCI_B0_CCR0_TE_Msk);
	while (data->sci.p_reg->CESR_b.TIST != 0U) {
	}
}
...
static int uart_ra_sci_b_async_rx_disable(const struct device *dev)
{
	...
	data->sci.p_reg->CFCLR_b.RDRFC = 1U;
...
#define UART_RA_SCI_B_IRQ_CONFIG_INIT(index)                                                       \
	do {                                                                                       \
		...
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(index, rxi, irq),                                  \
			    DT_INST_IRQ_BY_NAME(index, rxi, priority), uart_ra_sci_b_rxi_isr,      \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(index, txi, irq),                                  \
			    DT_INST_IRQ_BY_NAME(index, txi, priority), uart_ra_sci_b_txi_isr,      \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(index, tei, irq),                                  \
			    DT_INST_IRQ_BY_NAME(index, tei, priority), uart_ra_sci_b_tei_isr,      \
			    DEVICE_DT_INST_GET(index), 0);                                         \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(index, eri, irq),                                  \
			    DT_INST_IRQ_BY_NAME(index, eri, priority), uart_ra_sci_b_eri_isr,      \
			    DEVICE_DT_INST_GET(index), 0);                                         \
	} while (0)
