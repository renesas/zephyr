...
static int uart_rx_sci_async_tx_abort(const struct device *dev)
{
	...
	err = dtc_renesas_rx_infoGet(dev, data->txi_irq, &tx_properties);
...
static int uart_rx_sci_async_rx_disable(const struct device *dev)
{
	...
	err = dtc_renesas_rx_infoGet(dev, data->rxi_irq, &rx_properties);
	if (err != 0) {
		err = -EIO;
		goto unlock;
	}
...
#define UART_RX_SCI_ASYNC_INIT(index)                                                              \
	.dtc_enabled = DT_PROP(DT_INST_PARENT(index), dtc_enabled),                                \
	.dtc = DEVICE_DT_GET(DT_NODELABEL(dtc)),                                                   \
	.rx_transfer_info =                                                                        \
		{                                                                                  \
