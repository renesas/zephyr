...
static inline void ra_display_pm_policy_state_lock_get(void)
{
	pm_policy_state_lock_get(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
	pm_policy_state_lock_get(PM_STATE_STANDBY, PM_ALL_SUBSTATES);
}

static inline void ra_display_pm_policy_state_lock_put(void)
{
			    ...
			    const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	if (vsync_wait) {
		ra_display_pm_policy_state_lock_get();
		k_sem_take(&data->frame_buf_sem, K_FOREVER);
		ra_display_pm_policy_state_lock_put();
	}
...
static int display_suspend(const struct device *dev)
{
	...
	err = clock_control_off(config->clock_dev, (clock_control_subsys_t)&config->clock_glcdc_subsys);
	if (err) {
