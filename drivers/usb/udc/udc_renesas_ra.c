...
static int udc_event_xfer_setup(const struct device *dev, struct udc_renesas_ra_evt *evt)
{
	...
	if (setup_packet->bRequest == USB_SREQ_SET_ADDRESS) {
		data->ignore_status_in = true;
	} else if (USB_REQTYPE_GET_DIR(setup_packet->bmRequestType) == USB_REQTYPE_DIR_TO_DEVICE &&
...
static void udc_event_xfer_ctrl_in(const struct device *dev, struct net_buf *const buf)
{
	...
	if (bi->status) {
		if (!data->ignore_status_in) {
			/* Complete Status IN stage for control requests without
			 * Data OUT stage other than SET_ADDRESS.
			 */
...
#define UDC_RENESAS_RA_DEVICE_DEFINE(n)                                                            \
	...
	static struct udc_data udc_data_##n = {                                                    \
		...
		.priv = &udc_priv_##n,                                                             \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, udc_renesas_ra_driver_preinit, PM_DEVICE_DT_INST_GET(n),          \
