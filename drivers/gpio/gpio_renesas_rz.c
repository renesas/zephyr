...
#define GPIO_RZ_INT_CONNECT(idx, inst)                                                             \
	IRQ_CONNECT(DT_IRQ_BY_IDX(inst, idx, irq), DT_IRQ_BY_IDX(inst, idx, priority),             \
		    rz_gpio_isr##idx, DEVICE_DT_GET(inst), 0);

#define GPIO_RZ_INT_CONNECT_FUNC(inst)                                                             \
	static void rz_gpio_int_connect_func##inst(void)                                           \
	{                                                                                          \
		LISTIFY(DT_NUM_IRQS(inst),                        \
			GPIO_RZ_INT_CONNECT,                      \
			(;), inst)                                \
	}
/* Initialize GPIO device */
#define GPIO_RZ_INT_INIT(inst)                                                                     \
	GPIO_RZ_ALL_ISR_DEFINE(DT_NUM_IRQS(inst))                                                  \
	GPIO_RZ_INT_CONNECT_FUNC(inst)                                                             \
	static const struct gpio_rz_int_config rz_gpio_tint_cfg_##inst = {                         \
		.gpio_int_init = rz_gpio_int_connect_func##inst,                                   \
	};                                                                                         \
	static struct gpio_rz_int_data rz_gpio_int_data_##inst = {};                               \
	DEVICE_DT_DEFINE(inst, gpio_rz_int_init, NULL, &rz_gpio_int_data_##inst,                   \
			 &rz_gpio_tint_cfg_##inst, POST_KERNEL,                                    \
