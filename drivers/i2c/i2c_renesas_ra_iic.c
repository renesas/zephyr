...
struct i2c_ra_iic_config {
	...
	double master_rise_time_s;
	double master_fall_time_s;
	uint32_t master_noise_filter_stage;
	uint32_t master_duty_cycle_percent;
	uint32_t max_bitrate_supported;
};

struct i2c_ra_iic_data {
	iic_master_instance_ctrl_t master_ctrl;
	i2c_master_cfg_t master_fconfig;
	i2c_master_event_t master_event;
	iic_master_extended_cfg_t iic_master_ext_cfg;
#ifdef CONFIG_I2C_TARGET
	iic_slave_instance_ctrl_t slave_ctrl;
	i2c_slave_cfg_t slave_fconfig;
	i2c_slave_event_t slave_event;
	iic_slave_extended_cfg_t iic_slave_ext_cfg;
	struct i2c_target_config *target_cfg;
	bool transaction_completed;
#ifdef CONFIG_I2C_TARGET_BUFFER_MODE
	uint8_t slave_buf[CONFIG_I2C_TARGET_RENESAS_RA_IIC_BUFFER_SIZE];
#else
	uint8_t slave_buf;
#endif /* CONFIG_I2C_TARGET_BUFFER_MODE */
	struct i2c_ra_iic_fsp_irq fsp_irq;
...
static int calc_iic_master_clock_setting(const struct device *dev, const uint32_t master_rate,
					 iic_master_clock_settings_t *clk_cfg);
#ifdef CONFIG_I2C_TARGET
static int calc_iic_slave_clock_setting(const struct device *dev, const uint32_t slave_rate,
...
static int i2c_ra_iic_configure(const struct device *dev, uint32_t dev_config)
{
	...
	if (desire_bitrate <= config->max_bitrate_supported) {
		data->master_fconfig.rate = desire_bitrate;
	} else {
	...
	err = calc_iic_master_clock_setting(dev, data->master_fconfig.rate,
	...
	fsp_err = R_IIC_MASTER_Close(&data->master_ctrl);
	if (fsp_err != FSP_SUCCESS) {
		LOG_ERR("Failed to close I2C master instance. FSP_ERR=%d", fsp_err);
		return -EIO;
	}
	fsp_err = R_IIC_MASTER_Open(&data->master_ctrl, &data->master_fconfig);
	if (fsp_err != FSP_SUCCESS) {
			       ...
			       uint16_t addr)
{
	...
	R_IIC_MASTER_SlaveAddressSet(&data->master_ctrl, addr, addr_mode);
	...
	while (num_msgs > 0) {
		...
		if (current->flags & I2C_MSG_READ) {
			fsp_err =
				R_IIC_MASTER_Read(&data->master_ctrl, current->buf, current->len,
						  next != NULL && (next->flags & I2C_MSG_RESTART));
		} else {
			fsp_err =
				R_IIC_MASTER_Write(&data->master_ctrl, current->buf, current->len,
						   next != NULL && (next->flags & I2C_MSG_RESTART));
		}
...
static void i2c_ra_iic_master_callback(i2c_master_callback_args_t *p_args)
{
	const struct device *dev = p_args->p_context;
	struct i2c_ra_iic_data *data = dev->data;

	data->master_event = p_args->event;
...
static void i2c_ra_iic_slave_callback(i2c_slave_callback_args_t *p_args)
{
	const struct device *dev = p_args->p_context;
	struct i2c_ra_iic_data *data = dev->data;
	const struct i2c_target_callbacks *slave_cb = data->target_cfg->callbacks;
	fsp_err_t fsp_err;
	int err;
#ifdef CONFIG_I2C_TARGET_BUFFER_MODE
	uint8_t *buf = NULL;
	...
	case I2C_SLAVE_EVENT_RX_COMPLETE:
		if (p_args->bytes > 0) {
			slave_cb->buf_write_received(data->target_cfg, data->slave_buf,
						     p_args->bytes);
		}
		__fallthrough;
	case I2C_SLAVE_EVENT_TX_COMPLETE:
		if (data->transaction_completed) {
			slave_cb->stop(data->target_cfg);
		}
		break;
	case I2C_SLAVE_EVENT_RX_REQUEST:
		err = slave_cb->write_requested(data->target_cfg);
		if (err == 0) {
			fsp_err == R_IIC_SLAVE_Read(&data->slave_ctrl, data->slave_buf,
						    sizeof(data->slave_buf));
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
		} else {
			R_IIC_SLAVE_Read(&data->slave_ctrl, data->slave_buf, 0);
			LOG_DBG("Error is returned when calling write_received, send NACK to "
				"master");
		}
		data->transaction_completed = false;
		break;
	case I2C_SLAVE_EVENT_TX_REQUEST:
		err = slave_cb->buf_read_requested(data->target_cfg, &buf, &len);
		if (err == 0) {
			if (buf != NULL && len != 0) {
				fsp_err == R_IIC_SLAVE_Write(&data->slave_ctrl, buf, len);
				__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
			} else {
				LOG_ERR("buf is NULL or len is 0, Master will read 0xFF for the "
					"remaining bytes");
			}
		} else {
			LOG_DBG("Error is returned when calling buf_read_requested, Master will "
				"read 0xFF for the remaining bytes");
		}
		data->transaction_completed = false;
		break;
	case I2C_SLAVE_EVENT_RX_MORE_REQUEST:
		R_IIC_SLAVE_Read(&data->slave_ctrl, data->slave_buf, 0);
		LOG_ERR("The buffer is full, slave cannot receive more data. Please increase "
			"I2C_TARGET_RENESAS_RA_IIC_BUFFER_SIZE");
		slave_cb->stop(data->target_cfg);
		break;
	case I2C_SLAVE_EVENT_TX_MORE_REQUEST:
		LOG_ERR("Out of data to send to the master, Master will read 0xFF for the "
			"remaining bytes");
		break;
	...
	case I2C_SLAVE_EVENT_RX_COMPLETE:
		if (p_args->bytes > 0) {
			slave_cb->write_received(data->target_cfg, data->slave_buf);
		}
		__fallthrough;
	case I2C_SLAVE_EVENT_TX_COMPLETE:
		if (data->transaction_completed) {
	...
	case I2C_SLAVE_EVENT_RX_REQUEST:
		...
		if (err == 0) {
			...
			fsp_err == R_IIC_SLAVE_Read(&data->slave_ctrl, &data->slave_buf, 1);
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
		} else {
		...
		break;
	case I2C_SLAVE_EVENT_TX_REQUEST:
		err = slave_cb->read_requested(data->target_cfg, &data->slave_buf);
		if (err == 0) {
			fsp_err == R_IIC_SLAVE_Write(&data->slave_ctrl, &data->slave_buf, 1);
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
		} else {
			LOG_DBG("Error is returned when calling buf_read_requested, Master will "
				"read 0xFF for the remaining bytes");
		...
		break;
	case I2C_SLAVE_EVENT_RX_MORE_REQUEST:
		err = slave_cb->write_received(data->target_cfg, data->slave_buf);
		if (err == 0) {
			/* Continue to receive data */
			fsp_err == R_IIC_SLAVE_Read(&data->slave_ctrl, &data->slave_buf, 1);
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
		...
		break;
	case I2C_SLAVE_EVENT_TX_MORE_REQUEST:
		err = slave_cb->read_processed(data->target_cfg, &data->slave_buf);
		if (err == 0) {
			fsp_err == R_IIC_SLAVE_Write(&data->slave_ctrl, &data->slave_buf, 1);
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
		} else {
...
static int i2c_ra_iic_init(const struct device *dev)
{
	...
	switch (data->master_fconfig.rate) {
	case I2C_MASTER_RATE_STANDARD:
	case I2C_MASTER_RATE_FAST:
	case I2C_MASTER_RATE_FASTPLUS:
		ret = calc_iic_master_clock_setting(dev, data->master_fconfig.rate,
						    &data->iic_master_ext_cfg.clock_settings);
		if (ret != 0) {
			LOG_ERR("Failed to calculate I2C clock settings");
			break;
		...
		data->master_fconfig.p_extend = &data->iic_master_ext_cfg;
		break;
	default:
		LOG_ERR("%s: Invalid I2C speed rate: %d", __func__, data->master_fconfig.rate);
		return -ENOTSUP;
	}
...
#ifdef CONFIG_I2C_TARGET
	data->slave_fconfig.p_extend = &data->iic_slave_ext_cfg;
#endif /* CONFIG_I2C_TARGET */
...
{
	const uint32_t master_noise_filter_stage = config->master_noise_filter_stage;
	const double master_rise_time_s = config->master_rise_time_s;
	const double master_fall_time_s = config->master_fall_time_s;
	const uint32_t requested_duty = config->master_duty_cycle_percent;
	 ...
	 */
	if (divider == 0) {
		constant_add = 3 + master_noise_filter_stage;
	} else {
		/* All dividers other than 0 use an addition of 2 + master_noise_filter_stages. */
		constant_add = 2 + master_noise_filter_stage;
	}

	/* Converts all divided numbers to double to avoid data loss. */
	divided_pclk = (peripheral_clock >> divider);

	result->bitrate = 1 / ((total_brl_brh + 2 * constant_add) / divided_pclk +
			       master_rise_time_s + master_fall_time_s);
	result->duty = 100 * ((master_rise_time_s + ((brh + constant_add) / divided_pclk)) /
			      (master_rise_time_s + master_fall_time_s +
			       ((total_brl_brh + 2 * constant_add)) / divided_pclk));
					 ...
					 iic_master_clock_settings_t *clk_cfg)
{
	...
	case I2C_MASTER_RATE_FAST:
	case I2C_MASTER_RATE_FASTPLUS:
		requested_bitrate = master_rate;
		break;
	default:
		LOG_ERR("%s: Invalid I2C speed rate: %d", __func__, master_rate);
		return -EINVAL;
	}
	...
	min_brh = master_noise_filter_stage + 1;
	...
	for (int temp_divider = 0; temp_divider <= 7; ++temp_divider) {
		...
		total_brl_brh = ceil(((1 / (double)requested_bitrate) -
				      (master_rise_time_s + master_fall_time_s)) *
					     divided_pclk -
				     (2 * constant_add));
					...
					iic_slave_clock_settings_t *clk_cfg)
{
	...
	uint32_t min_brl = data->iic_slave_ext_cfg.clock_settings.digital_filter_stages + 1;
	uint32_t brl = ceil(peripheral_clock * requested_delay_ns / 1000000000);
...
static int i2c_ra_iic_target_register(const struct device *dev, struct i2c_target_config *cfg)
{
	...
	if (data->master_ctrl.open == 0) {
		LOG_ERR("%s: I2C master instance is not opened.", __func__);
		ret = -EIO;
		goto RELEASE_BUS;
	}
	...
	fsp_err = R_IIC_MASTER_Close(&data->master_ctrl);
	if (fsp_err != FSP_SUCCESS) {
		LOG_ERR("%s: Failed to close I2C master instance. FSP_ERR=%d", __func__, fsp_err);
		ret = -EIO;
		goto RELEASE_BUS;
	}

	if (I2C_TARGET_FLAGS_ADDR_10_BITS & cfg->flags) {
		data->slave_fconfig.addr_mode = I2C_SLAVE_ADDR_MODE_10BIT;
	} else {
		data->slave_fconfig.addr_mode = I2C_SLAVE_ADDR_MODE_7BIT;
	}
	data->target_cfg = cfg;
	data->slave_fconfig.slave = cfg->address;
	data->slave_fconfig.rate = data->master_fconfig.rate;

	ret = calc_iic_slave_clock_setting(dev, data->slave_fconfig.rate,
					   &data->iic_slave_ext_cfg.clock_settings);
	if (ret != 0) {
		LOG_ERR("Failed to calculate I2C slave clock settings");
		ret = -EIO;
		goto RELEASE_BUS;
	}

	data->slave_fconfig.p_callback = i2c_ra_iic_slave_callback;

	fsp_err = R_IIC_SLAVE_Open(&data->slave_ctrl, &data->slave_fconfig);
	if (fsp_err != FSP_SUCCESS) {
		LOG_ERR("%s: Failed to enter I2C slave mode. Try to re-open master mode", __func__);
		fsp_err = R_IIC_MASTER_Open(&data->master_ctrl, &data->master_fconfig);
		if (fsp_err != FSP_SUCCESS) {
			LOG_ERR("Failed to re-open I2C master instance: %s", dev->name);
		}
		ret = -EIO;
	}
...
static int i2c_ra_iic_target_unregister(const struct device *dev, struct i2c_target_config *cfg)
{
	...
	if (data->slave_ctrl.open == 0) {
		LOG_ERR("%s: I2C slave instance is not opened.", __func__);
		ret = -EINVAL;
		goto RELEASE_BUS;
	}

	fsp_err = R_IIC_SLAVE_Close(&data->slave_ctrl);
	if (fsp_err != FSP_SUCCESS) {
		LOG_ERR("%s: Failed to close I2C slave instance. FSP_ERR=%d", __func__, fsp_err);
		ret = -EIO;
		goto RELEASE_BUS;
	}
	...
	fsp_err = R_IIC_MASTER_Open(&data->master_ctrl, &data->master_fconfig);
	if (fsp_err != FSP_SUCCESS) {
		LOG_ERR("Failed to re-open I2C device instance: %s", dev->name);
		return -EIO;
	}
RELEASE_BUS:
	k_mutex_unlock(&data->bus_mutex);
...
#define SLAVE_FCONFIG_DEFAULT(index)                                                               \
	.slave_fconfig =                                                                           \
		{                                                                                  \
			...
			.p_callback = i2c_ra_iic_slave_callback,                                   \
...
void iic_eri_isr(const struct device *dev)
{
	...
	if (data->slave_ctrl.open) {
		if (data->slave_ctrl.p_reg->ICSR2_b.STOP == 1) {
...
#define I2C_RA_IIC_INIT(index)                                                                     \
	...
	static const struct i2c_ra_iic_config i2c_ra_iic_config_##index = {                        \
		...
		.master_noise_filter_stage = 1, /* Cannot be configured. */                        \
		.master_rise_time_s = DT_INST_PROP(index, rise_time_ns) / 1000000000,              \
		.master_fall_time_s = DT_INST_PROP(index, fall_time_ns) / 1000000000,              \
		.master_duty_cycle_percent = DT_INST_PROP(index, duty_cycle_percent),              \
	...
	static struct i2c_ra_iic_data i2c_ra_iic_data_##index = {                                  \
		.master_fconfig =                                                                  \
			{                                                                          \
				...
				.p_callback = i2c_ra_iic_master_callback,                          \
				.p_context = (void *)DEVICE_DT_GET(DT_DRV_INST(index)),            \
			},                                                                         \
		SLAVE_FCONFIG_DEFAULT(index)};                                                     \
                                                                                                   \
