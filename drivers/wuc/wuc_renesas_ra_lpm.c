...
static int ra_wuc_init(const struct device *dev)
{
	...
	/*
	 * Snapshot DPSIFR registers at boot.
	 *
	 * These registers are NOT reset by the Deep Software Standby cancel
	 * reset (per datasheet), so they retain the wakeup cause. We capture
	 * them now, before any code that could modify DPSIER (which can
	 * spuriously set DPSIFR bits) runs.
	 *
	 * The dummy read before each write-0 satisfies the datasheet
	 * requirement to read before clearing, and provides the minimum
	 * 6 PCLKB cycle gap required after DPSIER modification.
	 *
...
DEVICE_DT_INST_DEFINE(0, ra_wuc_init, NULL, &wuc_data, NULL,
		      PRE_KERNEL_1, CONFIG_WUC_INIT_PRIORITY, &ra_wuc_api);
