...
static void phy_type_setting(const struct device *dev)
{
	...
	case 3: /* RGMII */
		*p_miicr_register = (R_ESWM_MIICR0_TXCIDE_Msk | 1);
		...
		R_ESWM->MIIRR =
			R_ESWM->MIIRR | (1 << (R_ESWM_MIIRR_RGRST_Pos + data->fsp_cfg.channel));
		break;
...
static int renesas_ra_eth_init(const struct device *dev)
{
	...
	(void)net_eth_mac_load(&config->mcfg, data->mac_addr);

	data->fsp_cfg.p_mac_address = data->mac_addr;
	...
	{                                                                                          \
		.queue_cfg =                                                                       \
			{                                                                          \
				...
				.ports = BIT(DT_INST_PROP(n, channel)),                            \
...
#define ETH_RENESAS_RA_INIT(n)                                                                     \
	...
	ETH_NET_DEVICE_DT_INST_DEFINE(n, renesas_ra_eth_init, NULL, &eth##n##_renesas_ra_data,     \
				      ...
				      &eth_renesas_ra_api, NET_ETH_MTU);
