...
#define DT_DRV_COMPAT renesas_rcarmcu_ethernet_etnf
...
struct eth_etnf_data {
	...
	struct net_if *iface;
	uint8_t mac_addr[6];
	K_KERNEL_STACK_MEMBER(rx_stack, CONFIG_ETH_RENESAS_RCARMCU_ETNF_RX_THREAD_STACK_SIZE);
...
#if defined(CONFIG_ETH_RENESAS_RCARMCU_ETNF_USE_ZEROCOPY)
...
static int eth_etnf_init(const struct device *dev)
{
	...
	k_thread_create(&data->rx_thread, data->rx_stack,
			CONFIG_ETH_RENESAS_RCARMCU_ETNF_RX_THREAD_STACK_SIZE, eth_etnf_rx_thread,
			(void *)dev, NULL, NULL, CONFIG_ETH_RENESAS_RCARMCU_ETNF_RX_THREAD_PRIORITY,
			0, K_NO_WAIT);
...
#if (CONFIG_ETH_RENESAS_RCARMCU_ETNF_CFG_QOS_SUPPORT)
...
#define ETH_ETNF_DEFINE_STATIC(n)                                                                  \
	...
	static const ether_t1s_tx_queue_type_etnf eth_etnf_tx_q_##n[] = {                          \
		[0] = {.eth_tx_queue_id = (uint8_t)ETHER_T1S_TX_BE,                                \
		       ...
		       IF_ENABLED(CONFIG_ETH_RENESAS_RCARMCU_ETNF_CFG_QOS_SUPPORT, (     \
			.tx_queue_shaper = {                               \
	...
	static const ether_cfg_t eth_etnf_fsp_cfg_##n = {                                          \
		.channel = n,                                                                      \
		.zerocopy = IS_ENABLED(CONFIG_ETH_RENESAS_RCARMCU_ETNF_USE_ZEROCOPY)               \
