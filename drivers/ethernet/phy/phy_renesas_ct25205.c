...
static int phy_renesas_ct25205_link_cb_set(const struct device *dev, phy_callback_t cb, void *user_data)
{
	struct renesas_ct25205_data *data = dev->data;
	struct phy_link_state state;
...
#define RENESAS_CT25205_PHY_INIT_DRIVER(idx)                        \
	...
	DEVICE_DT_INST_DEFINE(idx, &phy_renesas_ct25205_init, NULL,               \
							&renesas_ct25205_##idx##_data,                    \
							&renesas_ct25205_##idx##_config,                  \
							POST_KERNEL,                                    \
							CONFIG_PHY_INIT_PRIORITY,                       \
							&renesas_ct25205_phy_api);
